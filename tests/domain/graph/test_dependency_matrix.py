...
from src.domain.graph.matrix_to_hyper_adapter import (
    MatrixToHyperGraphAdapter,
    legacy_matrix_from_list,
)
...
class TestDependencyMatrixInit:
    def test_init_emits_deprecation_warning(self):
        with pytest.warns(DeprecationWarning, match="DependencyMatrix is deprecated"):
            DependencyMatrix([[-1]])
    ...
    def test_sanctioned_adapters_suppress_internal_matrix_deprecation_noise(self):
        graph = HyperAdjacencyGraph()
        graph.register_node("a", {"runtime_id": 0})

        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always")
            MatrixToHyperGraphAdapter.from_legacy_list([[-1]], {0: "a"})
            legacy_matrix_from_list([[-1]])
            GraphToMatrixAdapter(graph).get_to_child_dependency_list(0)

        assert not any(
            issubclass(item.category, DeprecationWarning)
            and "DependencyMatrix is deprecated" in str(item.message)
            for item in caught
        )
