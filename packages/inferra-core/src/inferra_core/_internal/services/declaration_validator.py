...
DECLARATION_VALIDATION_INTERNAL_ERROR = "DECLARATION_VALIDATION_INTERNAL_ERROR"
...
class DeclarationValidator:
    ...
    def validate_rule_text(self, rule_text: str, rule_name: str = "") -> DeclarationValidationResult:
        ...
        try:
            from inferra_core._internal.domain.rule_parser.rule_set_parser import RuleSetParser
            from inferra_core._internal.domain.rule_parser.rule_set_reader import RuleSetReader
            from inferra_core._internal.domain.rule_parser.rule_set_scanner import RuleSetScanner
            ...
            return DeclarationValidationResult(
                valid=False,
                errors=(DeclarationFinding(
                    code=DECLARATION_VALIDATION_INTERNAL_ERROR,
                    message=(
                        "Declaration validation could not be completed; "
                        "the rule must not be saved, activated, or promoted"
                    ),
                    node_name=rule_name or None,
                ),),
            )
