...
from inferra_core._internal.domain.fact_values import FactValueType
...
class RuleValidationService:
    ...
    Optional content-hash cache (active only with a caller-supplied clock):
    - Key: SHA-256 of rule_text
    - Value: (timestamp, ValidationResult)
    - Eviction: LRU when cache exceeds maxsize; TTL expiry (default 300s)

    With the default ``clock=None``, validation is deterministic and no cache
    entries are read or written. Hosts may supply a monotonic or wall clock for
    a process-local performance cache that is outside result semantics.
    """
    ...
    def _parse_rule_line(
        ...
        if expr_match:
            ...
            try:
                ...
                errors.append(ValidationError(
                    code="INVALID_CALC_EXPRESSION",
                    message=f"IS CALC expression is invalid or unsupported: {exc}",
                    line=line_number,
                    node_name=var_name,
                ))
    ...
    def _get_cached(self, content_hash: str) -> Optional[ValidationResult]:
        if self._clock is None or self._cache_maxsize <= 0:
            return None
    ...
    def _set_cached(self, content_hash: str, result: ValidationResult) -> None:
        ...
        if content_hash in self._cache:
            self._cache.move_to_end(content_hash)
            self._cache[content_hash] = (self._clock(), result)
            return
