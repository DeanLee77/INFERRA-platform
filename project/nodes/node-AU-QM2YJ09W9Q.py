...
class Node(metaclass=ABCMeta):
    ...
    def has_children(self, dependency_matrix: list[list[int]]) -> list[int]:
        """Return a list of child indices for this node."""
        # Get the row for this node; a value of -1 means a child relationship
        return [i for i, val in enumerate(dependency_matrix[self.get_node_id()]) if val == -1]


    def has_parents(self, dependency_matrix: list[list[int]]) -> list[int]:
        """Return a list of parent indices for this node."""
        node_id = self.get_node_id()
        return [i for i, row in enumerate(dependency_matrix) if row[node_id] == -1]
