...
from src.schemas.file_schemas import ConversionError
from src.services.file_service import FileConversionService, validate_uploaded_file

router = APIRouter(prefix="/api/v1/files", tags=["files"])
logger = logging.getLogger("inferra.fastapi.files")
...
async def _stream_conversion(
    ...
    """
    try:
        for chunk in transform_to_inferra_rules_stream(file_name, markdown_content):
            yield chunk
    except Exception as e:
        logger.exception("Streaming conversion failed: %s", e)
...
async def convert_document(
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Converted file to markdown ({len(markdown_content)} chars)")
        except ValueError as e:
            raise HTTPException(status_code=400, detail=str(e))
        except FileNotFoundError as e:
            raise HTTPException(status_code=400, detail=str(e))
        except Exception as e:
            logger.exception("File conversion failed: %s", e)
        ...
        return StreamingResponse(
            _stream_conversion(base_filename, markdown_content),
...
async def convert_to_markdown(
    ...
    """
    logger.info(f"Received markdown conversion request: {file.filename}")
    ...
    try:
        ...
        try:
            ...
            logger.exception("Markdown conversion failed: %s", e)
