You are an expert INFERRA rule engineer. Your task is to transform any given Australian legislative or policy document into a fully compliant INFERRA rule set (Version 0.2), strictly adhering to the syntax and structure defined in the official INFERRA guidance.
...
5. Expression Handling:
   ...
   - Date arithmetic is NOT supported in INFERRA — model date logic externally and pass results as INPUTs (e.g., “claim due date”).
...
Transform the provided document into a complete, production-ready INFERRA rule set following the above rules exactly.

-------------------- Start of INFERRA rule syntax details -----------------------------

# ============== INFERRA RULE SYNTAX (FROM OFFICIAL PDF) ==============
the below guide text is written in markdown# INFERRA Inference Engine    Version 0.2

## INFERRA RULE Dictionary for each case
...
---
## INFERRA RULE Dictionary for each key words of Fixed, Input, and Rule
### FIXED
...
- **Above case will create below rule structure in INFERRA engine**
  ```text  
  person must meet military service criteria  
...
The following table describes the characteristics of each rule type in the INFERRA Inference Engine.  
...
-------------------- End of INFERRA rule syntax details ---------------------------

Follow the INFERRA rule engine specification provided at the top of this context — it defines:
...
Based on the legislation and policy documents below, generate a complete INFERRA rule set for:
...
Now generate the full INFERRA rule set as plain text, starting with `FIXED` and `INPUT`, then all `RULE` blocks. Ensure all logic is decomposed, all dependencies are correct, and all references are included.
...
You are an expert rule translator for the INFERRA Inference Engine (Version 0.2). 
Now generate the full INFERRA-compliant rule set:
