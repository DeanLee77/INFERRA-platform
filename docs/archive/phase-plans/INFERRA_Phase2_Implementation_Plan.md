...
**Document Status:** **Phase 2 COMPLETE** v3.4 (All 29 tasks done; Task #8 bridge work is now implemented in Phase 2.5; performance baselines captured; feature flag matrix tested)
...
**Reconciled status:** Phase 2 remains code-complete, consistent with `AGENTS.md` and this document's v3.3 status. `src/domain/state/session_schema.py` has moved beyond the Phase 2 handoff and now reports `CURRENT_SCHEMA_VERSION = 5`.

**Completion gate update:** Under the current completion policy, Redis/Fuseki/Celery/Docker/k6-dependent acceptance criteria are not considered production-complete until the actual live-service runs pass. Existing unit and mocked integration coverage remains useful, but it is no longer enough to mark those operational criteria complete.

**Live validation evidence:** Local compose validation now passes for Docker, Redis, Fuseki, Celery, PostgreSQL, API health, metrics, abduction smoke, disabled induction fallback, and Grafana health. Dockerized k6 at 20 VUs passes, and Dockerized k6 at 500 VUs passes under the Phase 4 local-compose production profile.

**Current blockers carried forward:** Phase 2 Task #8/topological runtime migration has been implemented in Phase 2.5, the Phase 2.5 graph bridge blockers are now closed, the Phase 4 local-compose 500-VU k6 load gate is closed, local Phase 4 restart chaos passes, Phase 4 real LLM circuit proof is closed, and Phase 4 session/auth/import-boundary/alert artifacts now pass targeted checks. The remaining production blockers from this audit stream are staging chaos/load repetition, external secrets/auth-policy decisions, legacy/flag retirement, and Phase 5 induction hardening. The repository-wide coverage command `pytest --cov=src --cov-fail-under=97` remains above the 97% gate.
...
**Important bridge decision:** Phase 2 makes `HyperAdjacencyGraph` and `DependencyGraphPort` stable and preferred for new runtime work, but it does **not** delete `DependencyMatrix` outright. Phase 2.5 has since moved inference, topology, iterate, import merge, and rule-file persistence paths to graph-first traversal; remaining matrix use is deprecated compatibility surface, sanctioned adapters, and tests. See `INFERRA_Phase2_5_DependencyMatrix_Bridge_Plan.md`.

### 1.1 Core Objectives
- [x] Refactor `_back_propagating` to delegate BFS to Phase 1's `HyperAdjacencyGraph.back_propagate()` (cycle-guarded)
- [x] Resolve `topo_sort.py` runtime migration in Phase 2.5: public legacy matrix entry points are deprecated compatibility facades backed by `MLTopologicalSortStrategy`, and strict impacted-subgraph sorting remains in `IncrementalPropagator` via `graphlib.TopologicalSorter`
...
- [x] `topo_sort.py` public BFS/DFS entry points are deprecated graph-backed facades over `MLTopologicalSortStrategy`; `IncrementalPropagator._topo_sort_subgraph()` remains the graphlib-backed invalidation-aware path for impacted subgraphs
- [x] `dfs_topological_sort_with_record()` is preserved as a compatibility entry point and delegates to graph-native `MLTopologicalSortStrategy`; regression coverage includes record-aware ordering and deep sparse DAG traversal without recursion-limit failures
...
- [~] `import-linter` CI checks pass; zero cross-module violations — carried forward to Phase 4 CI/architecture hardening, not a Phase 2.5 graph bridge blocker
...
**Phase 2 is COMPLETE** (2026-05-04). All 29 tasks done; Task #8 was completed through the Phase 2.5 bridge. As of 2026-05-11, the system has completed **Phase 2.5: DependencyMatrix Runtime Retirement & Graph-First Migration** and moves into Phase 3/4 production hardening work.

**Phase 2.5 bridge deliverables:**
1. [x] `InferenceEngine` hot-path traversal and propagation use `DependencyGraphPort`
2. [x] Parser/scanner can emit a canonical `HyperAdjacencyGraph` for new rule sets
3. [x] Legacy dense matrix payloads still load through `MatrixToHyperGraphAdapter`
4. [x] New/updated rule persistence writes graph-native edge-list/snapshot data with schema version
5. [x] Runtime topological sort is graph-native; matrix topo sort remains legacy-only
6. [x] `DependencyMatrix` imports are limited to adapter, legacy migration, and tests
7. [x] 1k/5k/10k sparse graph benchmarks are captured and compared to matrix baseline
8. [x] All production ports use ABCMeta. Any remaining Protocol-based port definitions are converted or explicitly scheduled as blocking technical debt.
...
11. `topo_sort.py` public paths migrated to graph-backed `MLTopologicalSortStrategy`; `dfs_topological_sort_with_record()` preserved with regression coverage
...
| 8 | Migrate `topo_sort.py` public paths off dense matrix traversal | ✅ Done via Phase 2.5 | 2026-05-11 | Legacy `TopologicalSort` entry points now adapt matrix inputs into `DependencyGraphPort` and delegate BFS/DFS/DFS-with-record traversal to `MLTopologicalSortStrategy`; private dense-matrix helper algorithms are removed. `IncrementalPropagator._topo_sort_subgraph()` remains the graphlib-backed invalidation-aware path |
...
| 24 | Phase 2.5 DependencyMatrix bridge handoff | ✅ Done | 2026-05-11 | Bridge implementation is complete: runtime inference, topology, iterate, import merge, and rule-file persistence paths are graph-first; `DependencyMatrix` remains only as deprecated compatibility and adapter surface; 1k/5k/10k sparse graph benchmarks and baseline evidence are captured in `benchmarks/baseline_phase2_5_sparse.json`; full inventory remains in `INFERRA_Phase2_5_DependencyMatrix_Bridge_Plan.md` §9 |
