...
```document-status```: Reconciled v4.6 - local hardening proof plus local-prod overlay/secret and OTel log bridge present; production completion blocked by staging chaos repetition and external deployment decisions
...
**Implemented in code/tests:** `SessionStorePort`, `InMemorySessionStore`, `RedisSessionStore`, Redis handoff/stale-write validation, HTTP session mutation persistence, schema migration through v5, `LLMOrchestratorPort`, `RealLLMOrchestrator` with timeout/retry/circuit-open fallback/half-open recovery and generic guarded chat for adapter features, `NullLLMOrchestrator`, `MockLLMOrchestrator`, prompt sanitization, API key/JWT/rate-limit/CSRF middleware, session owner enforcement, OpenTelemetry instrumentation, OTel trace/span correlation in structlog events, Docker/monitoring artifacts, local production Compose overlay with generated secret files, multi-profile k6 runner, Prometheus alert rules, import-linter CI contracts, Phase 4 baseline artifact, and two frontend workspaces under `frontends/`.

**Live validation evidence:** `docker compose up -d --build` now succeeds after the Dockerfile prompt-copy fix; `/api/v1/health` reports Redis, Celery, Fuseki, database, Z3, and induction workers as `ok`; internal Fuseki ping, Redis `PING`, Celery `inspect ping`, PostgreSQL readiness, Grafana health, metrics exposure, reasoning smoke, and both frontend workspaces' `npm.cmd test`/`npm.cmd run check` passed. Dockerized k6 at 20 VUs passed, and the 500-VU production profile passed for 1 minute on the compose network with 100.00% checks, 0.00% HTTP failures, and 197.99ms overall p95 after the disabled LLM fallback path was made direct. The local Phase 4 restart chaos suite passed for Redis, Fuseki, and worker restarts, and Prometheus alert rules validate with `promtool`.

**Still partial or blocked:** staging chaos/load repetition, external secret-manager integration, OIDC/RBAC/tenant policy, feature flag retirement/legacy adapter removal, and CI repetition of the load profile remain open.

**Completion rule:** Phase 4 can be marked complete only after staging chaos/load, external secrets, auth-policy, legacy-removal, and CI repetition criteria are either implemented or explicitly deferred. Live Redis/Fuseki/Celery/Docker smoke, Redis session handoff tests, auth/owner/CSRF tests, import-linter contracts, alert artifact tests, real LLM circuit tests, promtool validation, local restart chaos, and the local 500-VU k6 production gate now pass. The coverage gate is closed at 97.01%.
...
### 1.1 Core Objectives
- [x] Define `SessionStorePort` ABCMeta port with `InMemorySessionStore` and `RedisSessionStore` implementations
- [x] Migrate session storage from in-memory dict to Redis-backed `SessionStore` with TTL, connection pooling, distributed locking, optimistic concurrency control, and handoff/stale-write validation
- [x] Implement session schema versioning (`CURRENT_SCHEMA_VERSION = 4`) with Phase 3 -> 4 migration logic *(code has advanced to schema v5)*
- [x] Define `LLMOrchestratorPort` ABCMeta port with `RealLLMOrchestrator`, `NullLLMOrchestrator`, and `MockLLMOrchestrator` implementations
- [x] Deploy LLM orchestration layer with timeout, retry, circuit breaker, input sanitization, and prompt template versioning
- [~] Deploy Vite frontend with dynamic form rendering, iterate progress, interactive PROV-O trace viewer, error boundaries, and offline resilience *(frontends exist; tests/checks pass; browser/e2e runtime validation still pending)*
- [x] Add JWT/API key authentication, session ownership (`owner_id`), rate limiting, and CSRF protection *(OIDC/RBAC/tenant policy remains an external deployment decision)*
- [x] Establish `structlog`-based structured logging with Phase 4 mandatory fields and OpenTelemetry tracing
- [~] Deliver Docker Compose deployment with health checks, service auth, network binding, and secret management *(compose health/readiness pass; env-based substitution, `.env.example`, `docker-compose.prod.yml`, and local Docker secret scripts exist; commercial secret-manager integration remains external)*
- [~] Conduct load testing (500 concurrent sessions), chaos testing, and graceful degradation validation *(20-VU k6 smoke, multi-profile k6 runner, and 500-VU production gate pass/present; chaos scripts and restart suite are artifact-tested; local restart chaos passed for Redis, Fuseki, and worker; staging repetition pending)*
- [~] Remove legacy adapters, enforce strict `import-linter` port contracts, retire feature flags *(import-linter contracts pass; legacy removal/flag retirement pending)*
- [~] Capture performance baselines (`benchmarks/baseline_phase4.json`) and validate LLM observability metrics *(baseline, live metrics/Grafana smoke, alert rules, and LLM circuit proof exist; staging metric-threshold tuning pending)*
...
| Load test: 500 concurrent sessions | <5% error rate, <1GB RAM per worker; local compose production gate passed on 2026-05-11 |
...
- [x] Docker Compose cold start < 2 mins with all health checks passing in the local profile
- [x] Load test: 500 concurrent sessions, P95 < 300ms, <5% error rate in the local compose production profile
- [ ] `benchmarks/baseline_phase4.json` captured and committed
- [x] `.env.example` committed, `.env` and generated local-prod secrets ignored by git
...
| 9 | structlog-based logging (not dictConfig), Phase 4 fields, and OTel trace/span correlation | 4.5 |
