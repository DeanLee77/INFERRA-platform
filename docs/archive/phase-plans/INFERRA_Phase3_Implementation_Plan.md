...
**Document Status:** Reconciled v4.4 - local post-reasoning and orchestrator factory code present; completion still blocked by live ontology workflow validation
...
**Implemented in code/tests:** `BackwardChainOrchestrator`, `LegacyOrchestrator`, feature-flagged `create_orchestrator()` factory, `InferenceContext`, `SessionManager`, `SessionManagerPort`, `QuestionStrategyPort`, `ConservativeQuestionStrategy`, deterministic convergence hashing, PROV-O generation through `rdflib.Graph`, `/trace` support, schema migration through v5, Phase 3 baseline artifact (`benchmarks/baseline_phase3.json`), `OntologyDeltaConsumer`, convergence metrics, and the async ontology post-reasoning publisher/task (`src/tasks/ontology_post_reasoner.py`).

**Still partial or blocked:** `origin_module` API enrichment is not fully proven. Phase 2.5 graph-first boundary work is closed, live Fuseki/Celery/Redis/Docker smoke validation passes in compose, and the Phase 4 local-compose 500-VU production-load gate passes. The remaining Phase 3 validation risk is path-specific live proof of the post-reasoning task, Redis delta event, and main-process semantic-layer injection under local-prod Compose. The coverage gate is closed at 97.01%.

**Completion rule:** Phase 3 can be marked complete only after live Fuseki/Celery/Docker validation passes for ontology/post-reasoning paths. The 2026-05-11 readiness and load evidence is acceptable evidence for service availability, but not yet for every ontology workflow acceptance task. The 2026-05-12 implementation closes the local code gap; local-prod Compose evidence remains required.
...
### 1.1 Core Objectives
- [x] Wrap `InferenceEngine` in `BackwardChainOrchestrator` + `SessionManager` (async, thread-safe, structlog-instrumented)
- [x] Define `InferenceContext` dataclass with all attributes referenced by orchestrator, session manager, and trace generator
- [~] Implement synchronous ontology pre-reasoning at session start *(semantic/cache plumbing exists; live Fuseki validation and session-start wiring need confirmation)*
- [~] Trigger asynchronous post-reasoning after answer ingestion; publish deltas as events (not direct FactStore writes from Celery workers) *(async pipeline code exists; live Celery/Fuseki validation is required)*
- [x] Formalize tiered fixed-point convergence criteria with deterministic hash, delta tracking, iteration cap, and `GOAL_REACHED` as sufficient condition
- [x] Generate PROV-O triples using `rdflib.Graph` (not string concatenation) for valid RDF/JSON-LD output
- [~] Integrate `fact_source` + `origin_module` into `/summary` & `/trace` API responses with full error schemas *(`fact_source` is present; `origin_module` still needs completion/proof)*
- [x] Extract `QuestionStrategy` with `should_ask()` + `select_next()` methods from `question_resolver.py` for future ontology/LLM gating
- [x] Add `LegacyOrchestrator` fallback and `src.infrastructure.orchestrator_factory.create_orchestrator()` for `HYBRID_ORCHESTRATOR=false`
- [x] Propagate `structlog` + correlation-ID to all Phase 3 modules with Phase 3-specific mandatory fields and events
- [x] Migrate session schema from Phase 2 (v2) to Phase 3 (v3) with convergence state + question strategy + PROV-O trace
- [x] Define `SessionManagerPort` ABCMeta port + contract test suite
- [~] Extend `/health` endpoint for Phase 3 dependencies (ontology reasoner, active sessions) *(health extensions exist; live dependency checks remain required)*
- [~] Capture performance baselines (`benchmarks/baseline_phase3.json`) and validate convergence observability metrics *(baseline exists; live observability validation remains required)*
- [~] Enforce Phase 2.5 graph-first boundary: no new Phase 3 runtime code depends on `DependencyMatrix` *(guardrails exist; Phase 2.5 remains open)*
...
- [x] `LegacyOrchestrator` fallback works for `HYBRID_ORCHESTRATOR=false` and the factory selects `BackwardChainOrchestrator` when `HYBRID_ORCHESTRATOR=true`

### Ontology Pre/Post-Reasoning
- [ ] Pre-reasoning populates `FactSource.SEMANTIC` layer synchronously at session start
- [x] Async post-reasoning publishes `OntologyDeltaEvent`; main process injects under session lock (NO direct FactStore writes from Celery workers)
- [x] Dead-letter queue captures permanent failures; `structlog` error events emitted with correlation-ID
- [~] Circuit breaker/retry protection on ontology writes *(Celery retry + DLQ exist; dedicated circuit-breaker threshold policy still needs local-prod drill)*
